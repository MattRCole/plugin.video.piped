...
# ttml2ssa = cast(ttml2ssa.Ttml2Ssa, ttml2ssa)
...
def get_subtitle_from_piped(subtitle: StreamSubtitle, stream: Stream) -> Optional[str]:
...
def play_video(path):
    ...
    def reducer(
            ...
            item: Stream,
            /,
            get_quality: Callable[[Stream], int],
            quality_limit: Optional[int] = None,
        ):
        # if item["videoOnly"] == True:
        #     return acc
        quality = get_quality(item)
        if quality_limit is not None and quality > quality_limit:
            return acc
        if quality >= acc["quality"]:
            return {
                "quality": quality,
                "stream": item
            }
    ...
    stream_choice = functools.reduce(
        functools.partial(reducer, get_quality=lambda s: int(s['quality'][:-1]), quality_limit=1080),
        stream_info['videoStreams'],
        initial_acc,
    )
    # audio_choice = functools.reduce(
    #     functools.partial(reducer, key="bitrate", get_quality=lambda s: s['bitrate']),
    #     stream_info['audioStreams'],
    #     initial_acc,
    # )

    if stream_choice["quality"] == 0:
        raise ValueError("could not find valid stream")

    stream = stream_choice["stream"]
    # audio_stream = audio_choice["stream"]

    subtitle = next(
        (subtitle for subtitle in stream_info['subtitles'] if 'en' in subtitle['code'] and subtitle['autoGenerated'] is False),
        None
    )
    ...
    if subtitle is not None:
        subtitle_path = get_subtitle_from_piped(subtitle, stream)
        if subtitle_path is not None:
            play_item.setSubtitles([subtitle_path])


    play_item.setArt({
        "thumb": stream_info["thumbnailUrl"]
    })
    # videoTag = play_item.getVideoInfoTag()
    # audio_detail = xbmc.AudioStreamDetail(channels=2, codec=audio_stream["codec"])
    # audio_detail.

    # videoTag.addAudioStream()
    
    # xbmcplugin.setResolvedUrl(handle=_HANDLE, succeeded=True, listitem=play_item)

    xbmc.Player().play(stream_info["hls"], listitem=play_item)
